...
const trainerRequest = async (req, res) => {
  ...
  if (req.userInfo?.isVerifyed && req.userInfo?.role === "Trainer")
    return res.send({
      success: false,
      statusCode: 400,
      message: `You already be a verifyed Trainer`,
    });
  const checkRequst = await RoleRequestDB.findOne({
    user: req.userInfo._id,
  ...
  if (checkRequst)
    return res.send({
      success: false,
      statusCode: 400,
      message: `You already submitted a trainer request`,
      errorDetails:
        "Before your current request Rejected or Accepted you can't make another request",
    });
  await RoleRequestDB.create({
...
module.exports = { updateProfile, profile, traineeRequest, trainerRequest };
